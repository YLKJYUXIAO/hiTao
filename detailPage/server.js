...
function getGoosListObj(){
	...
	if(!goodsListStr){  //如果没有
		$.cookie("goodsList","[]",{expires:7,path:"/"});
...
function getKind(){
	var goodsListObj = getGoosListObj();
	var sum = goodsListObj.length;
	$.cookie("goodsKind",sum,{expires:7,path:"/"});
}
/*
 	根据id查找本是否含有指定的商品
 	参数：id  商品的编号
...
function  addGoodsToCart(goods){
	...
	$.cookie("goodsList",goodListStr,{expires:7,path:"/"});
}
...
function updateGoodsById(id,num){ 	
	...
	$.cookie("goodsList",goodsListStr,{expires:7,path:"/"});
}
/*
 	删除本地数据
